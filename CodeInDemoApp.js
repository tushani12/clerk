...
//Updated Function with Dynamic Programming
    ...
    function getSelectedDuration() {
        ...
        if (post && lang) {
            ...
            return durationMap[postVal]?.[langVal] || 10; // default 10 min if not found
        }
        return 10; // fallback
    }
    ...
    function performCompleteAnalysis() {
        ...
        for (const w of comprehensiveAligned) {
            ...
            if (w.status === 'correct') {
                ...
                style = "color:white; background:#DB473C; padding:2px 4px; border-radius:3px;";
                const correctWord = w.expected || "";
                const correctWordStyle = "color:#e5dedd;  background:#6aa84f; font-style:italic; padding:2px 4px; border-radius:3px;";  // Grey color
                htmlOutput += `<span style="${style}">${w.word} <span style="${correctWordStyle}">(${correctWord})</span></span> `;                continue;  // Prevent default logic from running
            } else if (w.status === 'missing') {
